...
export class BusquedaRegistrosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  busquedaTForm: FormGroup = this.fb.group({
    fechai: new FormControl(null),
    fechaf: new FormControl(null),
    cedula: ['', [Validators.pattern(/^\S.*$/)]],
  ...
  busquedaTransaccionesAvanzadotxt() {
    ...
    this.AdministradorService.consultarFechaTransacciones({
      fechai: this.busquedaTForm.value.fechai ? moment(this.busquedaTForm.value.fechai).format("DD/MM/YYYY") : null,
      fechaf: this.busquedaTForm.value.fechaf ? moment(this.busquedaTForm.value.fechaf).format("DD/MM/YYYY") : null,
      ...
      next: (data) => {
        ...
        if (data.code !== 9999) {
          ...
          this.dataSource.data = (data.data.data || []).map((item: any) => ({
            ...item,
            estado_nombre: item.estadoNombre,
            descRespuestaMainframe: item.descripcionRespuestaMainframe || ''
          }));
  ...
  async busquedaTransaccionesAvanzado() {
    ...
    this.AdministradorService.consultarFechaTransacciones(params).subscribe(
      (data) => {
  ...
  async exportToExcel() {
    ...
    worksheet.getColumn(15).width = 50; // Nueva columna
    ...
    workbook.xlsx.writeBuffer().then((buffer: any) => {
      ...
      saveAs(blob, 'ReporteTransacciones_' + moment(new Date()).format("DD_MM_YYYY") + '.xlsx');
    });
  }

  clear() {
    this.busquedaTForm.reset();
    this.busquedaTransacciones();
  }
